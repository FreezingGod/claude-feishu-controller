...
> 通过飞书或 Discord 远程控制 Claude Code，实时接收状态通知和消息推送
...
本项目支持通过 **飞书** 或 **Discord** 实现远程桥接，解决上述问题。两个平台功能完全对等，可根据需要选择其一或同时运行。
...
| 功能 | 说明 |
|------|------|
| **多平台支持** | 同时支持飞书和 Discord 两个消息平台 |
| **状态监控** | 自动检测 Claude Code 的各种状态并推送通知 |
| **消息同步** | 监控 transcript.jsonl 文件，实时将 Claude 的回复推送到消息平台 |
| **远程交互** | 通过消息平台回复确认、选择选项、发送命令 |
...
│                         桥接服务 (Node.js)                          │
│  ┌──────────┐  ┌──────────┐  ┌──────────────┐  ┌──────────┐       │
│  │  Config  │  │ Monitor  │  │  Messenger   │  │ Handlers │       │
│  └──────────┘  └────┬─────┘  │ ┌──────────┐ │  └────┬─────┘       │
│                      │       │ │  Feishu   │ │       │             │
│                      │       │ │  Discord  │ │       │             │
│                      │       │ └──────────┘ │       │             │
...
│   └── buffer.js        # 缓冲区管理
│
├── messenger/           # 消息发送（多平台适配）
│   ├── adapter.js       # MessengerAdapter 接口定义
│   ├── feishu.js        # 飞书适配器实现
│   ├── discord.js       # Discord 适配器实现
│   └── index.js         # 模块导出
│
├── handlers/            # 消息处理
│   ├── command.js       # 各命令处理器实现
...
├── discord-proxy-bootstrap.mjs # Discord 代理引导模块
├── index.js                    # 飞书模式入口
└── index-discord.js            # Discord 模式入口
```
...
- Node.js >= 18.0.0
- tmux
- 飞书企业自建应用（飞书模式）或 Discord Bot（Discord 模式）
...
4. **重要**：在 Bot 页面开启 **MESSAGE CONTENT INTENT**（Privileged Gateway Intents 下方）

#### 步骤 2：邀请 Bot 到服务器

1. 进入左侧 **OAuth2 → URL Generator**
2. Scopes 勾选 `bot`
3. Bot Permissions 勾选：`Send Messages`、`Read Message History`、`Embed Links`
...
将 Bot Token 和频道 ID 填入 `.env` 文件：

```env
DISCORD_BOT_TOKEN=MTxxxxxxxxxxxxxxx.xxxxxx.xxxxxxxxxxxxxxxxxxxxxxxxxx
...
# PM2
pm2 start src/index.js --name claude-bridge-feishu
```
...
# 后台运行
nohup npm run start:discord >> server-discord.log 2>&1 &

# PM2
pm2 start src/index-discord.js --name claude-bridge-discord \
  --node-args="--import ./src/discord-proxy-bootstrap.mjs"
```

> **代理说明**：Discord 模式通过 `discord-proxy-bootstrap.mjs` 自动支持 HTTP 代理。如果需要代理访问 Discord，设置环境变量 `HTTPS_PROXY=http://your-proxy:port` 即可，REST API 和 WebSocket 连接都会自动走代理。
...
# PM2
pm2 stop claude-bridge-feishu   # 飞书模式
pm2 stop claude-bridge-discord  # Discord 模式
```
...
> **注意**：其他状态（如 plan_mode、testing、git_operation、warning）会被检测但**不发送通知**，仅在日志中记录。
...
# 3. 如果需要代理，确认 HTTPS_PROXY 已设置
# 4. 确认代理可以访问 discord.com 和 gateway.discord.gg
curl --proxy $HTTPS_PROXY https://discord.com/api/v10/gateway
curl --proxy $HTTPS_PROXY https://gateway.discord.gg/
# 5. 查看服务日志
tail -f server-discord.log
```

### 问题：Discord Bot 连接后收不到消息

```bash
# 1. 确认 Bot 已加入服务器且在目标频道有权限
# 2. 确认 DISCORD_CHANNEL_ID 正确
# 3. 确认 Bot 页面已开启 MESSAGE CONTENT INTENT（否则收到的消息内容为空）
```
...
// 查看捕获的内容
// 在飞书/Discord 中发送: /show

// 查看当前状态
// 在飞书/Discord 中发送: /status

// 查看配置
// 在飞书/Discord 中发送: /config
```
